...
def create_data_part(opt):
    ...
    train_ds = SPAQDataset(train_csv_path, opt["path_to_images"], if_train=True, rotate=False, hor=False, ver=False)
...
def start_train(opt):
    ...
    for epoch in range(opt["num_epoch"]):
        ...
        if tsrcc > srcc_best or tlcc > lcc_best or tacc > acc_best:
            ...
            srcc_best = max(tlcc, lcc_best)
            srcc_best = max(tacc, acc_best)
            if tsrcc > 0.75:
                model_name = f"e_{epoch}_acc{tacc}_srcc{tsrcc}_lcc{tlcc}.pth"
