...
class DAttentionBaseline(nn.Module):
    ...
    def __init__(self, q_size, kv_size, n_heads, n_head_channels, n_groups, attn_drop, proj_drop, stride, offset_range_factor, use_pe, dwc_pe, no_off, fixed_pe, stage_idx):
        ...
        self.conv_offset = nn.Sequential(nn.Conv2d(self.n_group_channels, self.n_group_channels, kk, stride, kk // 2, groups=self.n_group_channels), LayerNormProxy(self.n_group_channels), nn.GELU(),
        ...
        self.ref_point14 = nn.Conv2d(self.nc, 2, kernel_size=1, stride=1, padding=0)

        self.ref_gate = nn.Conv2d(self.nc, 1, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x):
        ...
        x_sampled = F.grid_sample(
            ...
            mode='bilinear',
            align_corners=True)  # B * g, Cg, Hg, Wg
        ...
        if self.use_pe:
            ...
            if self.dwc_pe:
                ...
                attn_bias = F.grid_sample(input=rpe_bias.reshape(B * self.n_groups, self.n_group_heads, 2 * H - 1, 2 * W - 1), grid=displacement[..., (1, 0)], mode='bilinear', align_corners=True)  # B * g, h_g, HW, Ns
